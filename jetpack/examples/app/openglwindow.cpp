...
void OpenGLWindow::initializeGL() {
  ...
  // Create program to render the dino
  m_objectsProgram = createProgramFromFile(getAssetsPath() + "jetpack.vert",
                                        getAssetsPath() + "jetpack.frag");
...
void OpenGLWindow::restart() {
  ...
  m_score = 0;
  m_jetpack.initializeGL(m_objectsProgram);
...
void OpenGLWindow::update() {
  ...
  //m_jetpack.update(m_gameData, deltaTime);
  m_jetpack.update(m_gameData);
  m_blocks.update(m_jetpack, m_gameData, deltaTime);
...
void OpenGLWindow::paintUI() {
...

  {
    ...
    ImGui::Begin("Jetpack - The Game", nullptr, windowFlags);
    ...
    ImGui::SameLine();
    ImGui::RadioButton("Impossível", &m_blocks.m_delay, 200);
...
void OpenGLWindow::terminateGL() {
  ...
  m_jetpack.terminateGL();
}

void OpenGLWindow::checkCollisions() {

  // Verifica colisao entre jetpack e bloco
  for (auto &block : m_blocks.m_blocks) {
    if (block.m_dead) continue;

    auto distance{glm::distance(m_jetpack.m_translation, block.m_translation)};

    if (distance < m_jetpack.m_scale * 0.9f + block.m_scale * 0.85f) {
